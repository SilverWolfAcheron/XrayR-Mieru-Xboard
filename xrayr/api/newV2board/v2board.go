...
func New(apiConfig *api.Config) *APIClient {
	...
	apiNodeType := normalizeNodeType(apiConfig.NodeType)
	var nodeType string

	if apiNodeType == "V2ray" && apiConfig.EnableVless {
		nodeType = "vless"
	} else {
		nodeType = strings.ToLower(apiNodeType)
	}
	...
	apiClient := &APIClient{
		...
		MieruPort:           apiConfig.MieruPort,
		MieruTransport:      apiConfig.MieruTransport,
		MieruTrafficPattern: apiConfig.MieruTrafficPattern,
...
func (c *APIClient) parseResponse(res *resty.Response, path string, err error) (*simplejson.Json, error) {
	...
	if res == nil {
		return nil, fmt.Errorf("request %s failed: received nil response", c.assembleURL(path))
	}
...
func (c *APIClient) GetNodeInfo() (nodeInfo *api.NodeInfo, err error) {
	...
	res, reqErr := c.client.R().
		SetHeader("If-None-Match", c.eTags["node"]).
		ForceContentType("application/json").
		Get(path)

	// Etag identifier for a specific version of a resource. StatusCode = 304 means no changed
	if reqErr == nil && res != nil && res.StatusCode() == 304 {
		return nil, errors.New(api.NodeNotModified)
	}
	// update etag
	if res != nil && res.Header().Get("Etag") != "" && res.Header().Get("Etag") != c.eTags["node"] {
		c.eTags["node"] = res.Header().Get("Etag")
	}

	nodeInfoResp, err := c.parseResponse(res, path, reqErr)
	if err != nil {
		if c.NodeType == "Mieru" && c.MieruPort > 0 {
			log.Warnf("Xboard Mieru config failed, using local config: port=%d transport=%s; %v", c.MieruPort, c.MieruTransport, err)
			return c.localMieruNodeInfo()
		}
		return nil, err
	}
	b, _ := nodeInfoResp.Encode()
	if err := json.Unmarshal(b, server); err != nil {
		return nil, fmt.Errorf("decode node config failed: %w; body: %s", err, string(b))
	}
	...
	case "Mieru":
		nodeInfo, err = c.parseMieruNodeResponse(server)
	default:
...
func (c *APIClient) localMieruNodeInfo() (*api.NodeInfo, error) {
	server := &serverConfig{
		ServerPort: c.MieruPort,
	}
	server.Transport = c.MieruTransport
	server.TrafficPattern = c.MieruTrafficPattern
	return c.parseMieruNodeResponse(server)
}
...
func (c *APIClient) GetUserList() (UserList *[]api.UserInfo, err error) {
	...
	switch c.NodeType {
	case "V2ray", "Trojan", "Shadowsocks", "Vmess", "Vless", "Mieru":
		break
	default:
		return nil, fmt.Errorf("unsupported node type: %s", c.NodeType)
	}
	...
	// update etag
	if res != nil && res.Header().Get("Etag") != "" && res.Header().Get("Etag") != c.eTags["users"] {
		c.eTags["users"] = res.Header().Get("Etag")
	}

	usersResp, err := c.parseResponse(res, path, reqErr)
	if err != nil {
		return nil, err
	}
	b, _ := usersResp.Get("users").Encode()
	if err := json.Unmarshal(b, &users); err != nil {
		return nil, fmt.Errorf("decode user list failed: %w; body: %s", err, string(b))
	}
	if len(users) == 0 {
	...
	for i := 0; i < len(users); i++ {
		...
		if c.NodeType == "Shadowsocks" || c.NodeType == "Mieru" {
			u.Passwd = u.UUID
		}
		userList[i] = u
	}
	...
	for i := range routes {
		if routes[i].Action == "block" {
			ruleList = append(ruleList, api.DetectRule{
				ID:      i,
				Pattern: regexp.MustCompile(strings.Join([]string(routes[i].Match), "|")),
			})
		}
	}
...
// parseMieruNodeResponse parses Xboard UniProxy Mieru node config.
func (c *APIClient) parseMieruNodeResponse(s *serverConfig) (*api.NodeInfo, error) {
	transport := strings.ToUpper(s.Transport)
	if transport == "" {
		transport = "TCP"
	}
	if transport != "TCP" && transport != "UDP" {
		return nil, fmt.Errorf("unsupported mieru transport: %s", s.Transport)
	}
	return &api.NodeInfo{
		NodeType:            c.NodeType,
		NodeID:              c.NodeID,
		...
		MieruTrafficPattern: s.TrafficPattern,
		NameServerConfig:    s.parseDNSConfig(),
	}, nil
}
	...
	for i := range s.Routes {
		if s.Routes[i].Action == "dns" {
			nameServerList = append(nameServerList, &conf.NameServerConfig{
				...
				Domains: []string(s.Routes[i].Match),
			})
		}
	}
